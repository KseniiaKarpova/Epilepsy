...
'''
fig = go.Figure()
df = pd.read_csv('tests/987_tW/volt.csv', sep=',', encoding="UTF-7")
#df.head()
Time = df['t'].unique()
#print(df['v'])
...
fig.update_layout(
    ...
    scene = dict(
            xaxis = dict(nticks=4, range=[0,200],),
                         ...
                         zaxis = dict(nticks=4, range=[850,-850],),), #850 - -850    1000-1300
                        #zaxis = dict(nticks=4, range=[1000,1300],),),
    #width=700,
    #margin=dict(r=20, l=10, b=10, t=10)
    )

fig.write_html(os.path.join(outdir,'wave_test(ext).html'))
#fig.write_html(куда, "name")
#fig.show()
...
fig2 = go.Figure()
df2 = pd.read_csv('tests/985_60_tW/volt_gaba.csv', sep=',', encoding="UTF-7")
#df.head()
Time = df2['t'].unique()
# Add traces, one for each slider step
for step in Time:
