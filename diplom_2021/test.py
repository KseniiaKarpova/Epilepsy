...
outdir = os.path.abspath('tests/1027')
...
cells = [TCR(0,0,0,0),Spinstel4(4,4,4,1), SyppyrFRB(4,4,4,2), SyppyrRS(0,2,0,3), TuftIB5(5,5,5,4), TuftRS5(6,6,6,5),  NontuftRS6(10,10,10,6), Bask56(7,7,7,7), Axax56(8,8,8,8), LTS56(9,9,9,9)]
time = h.Vector().record(h._ref_t)

for i in range(len(cells)-1):
    cells[0].connect_cells(cells[i+1],1, 1, 2)
...
for j in range(0, 5):
    ...
    stim.number = 1
    stim.start = 5
    ncstim = h.NetCon(stim, cells[0].synlistexE[j])
...
#stim.number = 1
#stim.start = 10
#ncstim = h.NetCon(stim, cells[0].synlistexE[0])
#ncstim.delay = 1
#ncstim.weight[0] = 1
#stims.append(ncstim)
#stims.append(stim)
...
def run(tstop):
    ...
    if pcid == 0:
        for cell in cells:
            #plot_id(cell.data, ['1','2','3','4','5'],cell.id)
    ...
    #for n in cells:
    #    plt.figure()
    #    plt.vlines(n.spike_times,0,10)
    #    plt.show()
    for id,n in enumerate(cells):
        plot_spike_html(n,time,id)
    #with open(os.path.join(outdir, 'extr_all.csv'), "w") as csv_file:
